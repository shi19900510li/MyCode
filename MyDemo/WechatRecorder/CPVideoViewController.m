...
@interface CPVideoViewController () <AVCaptureFileOutputRecordingDelegate>
//轻触拍照，按住摄像
...
//视频播放
@property (strong, nonatomic) CPAVPlayer *player;

@property (strong, nonatomic) CPProgressview *progressView;
    ...
    if (IOS_10_OR_LATER) {
        if (@available(iOS 10.0, *)) {
            if (@available(iOS 10.2, *)) {
                NSArray<AVCaptureDeviceType> *deviceType = @[AVCaptureDeviceTypeBuiltInWideAngleCamera, AVCaptureDeviceTypeBuiltInDualCamera,AVCaptureDeviceTypeBuiltInMicrophone];
                AVCaptureDeviceDiscoverySession *audioCaptureDeviceSession = [AVCaptureDeviceDiscoverySession discoverySessionWithDeviceTypes:deviceType mediaType:AVMediaTypeAudio position:AVCaptureDevicePositionUnspecified];
                audioCaptureDeviceSession = [AVCaptureDeviceDiscoverySession discoverySessionWithDeviceTypes:deviceType mediaType:AVMediaTypeAudio position:AVCaptureDevicePositionUnspecified];
                audioCaptureDevice = [audioCaptureDeviceSession.devices firstObject];
            } else {
...
- (void)onEnsureAction:(UIButton *)sender {
    ...
    if (self.saveVideoUrl) {
        WS(weakSelf)
        [CPUtility showProgressDialogText:@"视频处理中..."];
        ...
        [assetsLibrary writeVideoAtPathToSavedPhotosAlbum:self.saveVideoUrl completionBlock:^(NSURL *assetURL, NSError *error) {
            ...
            if (error) {
                ...
                [CPUtility showAllTextDialog:[UIApplication sharedApplication].keyWindow Text:@"保存视频到相册发生错误"];
            } else {
...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error{
    ...
    if (self.isVideo) {
        ...
        if (!self.player) {
            self.player = [[CPAVPlayer alloc] initWithFrame:self.bgView.bounds withShowInView:self.bgView url:outputFileURL];
        } else {
            if (outputFileURL) {
