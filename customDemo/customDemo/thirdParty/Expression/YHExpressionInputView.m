...
- (instancetype)init {
    ...
    self.frame = CGRectMake(0, 0, kSelfView_Width, kViewHeight);
...
- (void)_initCollectionView {
    CGFloat itemWidth = (kSelfView_Width - 10 * 2) / 7.0;
    itemWidth = CGFloatPixelRound(itemWidth);
    CGFloat padding = (kSelfView_Width - 7 * itemWidth) / 2.0;
    CGFloat paddingLeft  = CGFloatPixelRound(padding);
    CGFloat paddingRight = kSelfView_Width - paddingLeft - itemWidth * 7;
    ...
    _collectionView = [[YHEmoticonScrollView alloc] initWithFrame:CGRectMake(0, 0, kSelfView_Width, kOneEmoticonHeight * 3) collectionViewLayout:layout];
    ...
    _pageControl.size = CGSizeMake(kSelfView_Width, 20);
...
- (void)_initBottomToolbar {
    ...
    _viewBotToolBar.size = CGSizeMake(kSelfView_Width, kToolbarHeight);
    ...
    CGSize size = CGSizeMake(kSelfView_Width-btnSendWidth, _viewBotToolBar.size.height);
    scroll.size = size;
    ...
    btnSend.left = kSelfView_Width - btnSendWidth;
    ...
    for (NSUInteger i = 0; i < _emoticonGroups.count; i++) {
        ...
        btn.left = (kSelfView_Width-btnSendWidth) / (float)_emoticonGroups.count * i;
        btn.tag = i;
...
- (UIButton *)_createToolbarButton {
    ...
    btn.size = CGSizeMake((kSelfView_Width - btnSendBtnWidth) / _emoticonGroups.count, kToolbarHeight);
